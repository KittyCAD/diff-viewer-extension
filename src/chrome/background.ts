...
import { KittycadUser, Message, MessageGetFileDiff, MessageGetGithubPullFilesData, MessageIds, MessageResponse, MessageSaveToken } from "./types";
...
async function initKittycadApi() {
    try {
        ...
        console.log(`Logged in on kittycad.io as ${(kittycadResponse as KittycadUser).email}`)
    } catch (e) {
...
chrome.runtime.onMessage.addListener((message: Message, sender: chrome.runtime.MessageSender,
    ...
    if (message.id === MessageIds.GetGithubUser) {
        github.rest.users.getAuthenticated().then(r => sendResponse(r.data)).catch(e => sendResponse(e))
        return true
    }

    if (message.id === MessageIds.GetKittycadUser) {
        users.get_user_self({ client: kittycad }).then(r => sendResponse(r as KittycadUser)).catch(e => sendResponse(e))
        return true
    }

    if (message.id === MessageIds.SaveGithubToken) {
        const { token } = message.data as MessageSaveToken
        saveGithubTokenAndReload(token).then(() => sendResponse({ token })).catch(e => sendResponse(e))
        return true
    }

    if (message.id === MessageIds.SaveKittycadToken) {
        const { token } = message.data as MessageSaveToken
        saveKittycadTokenAndReload(token).then(() => sendResponse({ token })).catch(e => sendResponse(e))
        return true
    }

    if (message.id === MessageIds.GetFileDiff) {
        const { owner, repo, sha, parentSha, file } = message.data as MessageGetFileDiff
        getFileDiff(github, kittycad, owner, repo, sha, parentSha, file).then((r) => sendResponse(r)).catch(e => sendResponse(e))
        return true
    }
})
